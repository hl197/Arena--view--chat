...
<p align="center">
  <img src="docs/screenshots/01-homepage.png" alt="首页提问" width="400">
  <img src="docs/screenshots/02-debate.png" alt="群聊讨论" width="400">
</p>

> 💡 想亲自体验？→ [快速开始](#快速开始)，`bash start.sh` 一键启动。
...
- **Round 1 并行研究**：开场陈述阶段 5 个 Agent 同时搜读（`asyncio.gather`），耗时从 300s 压缩到 ~60s
- **固定顺序发言**：Round 1 研究完成后按 `p_01→p_02→p_03→p_04→p_05` 顺序依次发言；Round 2+ 串行研究+发言
...
│   │   │   ├── DebatePage.tsx    # 群聊辩论页（两栏布局：侧边栏 + 聊天主区域）
│   │   │   └── SettingsPage.tsx  # 设置页
│   │   ├── components/
│   │   │   ├── chat/             # 聊天组件
│   │   │   │   ├── ChatHeader.tsx      # 聊天头部
│   │   │   │   ├── ChatInput.tsx       # 输入框
│   │   │   │   ├── MessageBubble.tsx   # 消息气泡（手绘手账风）
│   │   │   │   ├── AgentStatusBar.tsx  # Agent 状态栏
│   │   │   │   ├── TypingIndicator.tsx # 正在输入指示器
│   │   │   │   ├── TimeStamp.tsx       # 时间戳
│   │   │   │   ├── MessageContextMenu.tsx # 右键菜单
│   │   │   │   └── RippleEntrance.tsx  # 涟漪入场动效
│   │   │   ├── ui/               # 手绘手账风 UI 组件库
│   │   │   │   ├── HandDrawnButton.tsx
│   │   │   │   ├── HandDrawnCard.tsx
│   │   │   │   ├── HandDrawnAvatar.tsx
...
│   │   │   │   └── DecisionMapPanel.tsx  # 决策地图面板
│   │   │   └── sidebar/          # 侧边栏
│   │   │       ├── HistorySidebar.tsx    # 历史侧边栏
│   │   │       └── HistoryCard.tsx       # 历史卡片
│   │   ├── store/
│   │   │   ├── debateStore.ts   # Zustand 辩论状态管理
│   │   │   └── uiStore.ts       # UI 状态（侧边栏/面板）
│   │   ├── hooks/
...
```bash
bash start.sh       # Git Bash，自动安装依赖、启动后端 :8000 + 前端 :5173
# 或
...
- ✅ 顺序轮次多智能体群聊架构 + Round 1 并行化（5 人并发搜读）
- ✅ 6 种人格原型 + 双层角色建模
- ✅ SSE 全流程流式推送 + 消息延迟队列
- ✅ 手绘手账风 UI 体系（HandDrawn 组件库 + 侧边栏 + 面板）
- ✅ `start.sh` / `start.ps1` 一键启动脚本
- ✅ 对话历史 SQLite 持久化 + 历史回放（SSE 404 自动降级 REST API）
- ✅ `/debate` 浏览历史（无需新建对话即可查看历史侧边栏）
- ⏳ JWT 认证、API Key 加密存储
- ⏳ 额度持久化与限制、热门问题缓存
- ⏳ Docker 部署
